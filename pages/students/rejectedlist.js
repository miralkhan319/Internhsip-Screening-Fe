...
const rejected = () => {
 ...
 console.log(query)
      const response = await axios.get(`/student_registrations/rejected_students_pagedata?${query}`, {
        headers: {
 ...
 const getSerialNumber = (index) => {
  ...
  useEffect(() => {
    axios.get('/student_registrations/rejected_students_pagedata', {
      headers: {
        "Authorization": `Bearer ${session?.user.data.access_token}`
      }}).then((response) => {
      setData(response.data.pageDto);
  ...
  const postData2 = async () => {
    try {
      const response = await axios.patch(`/student_registrations/send_email_to_Rejected_Student_through_batch/${batchId}`, {
        headers: {
          "Authorization": `Bearer ${session?.user.data.access_token}`
        }});
  ...
  <path strokelinecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
        ...
        <div className="sm:col-span-2 flex items-center justify-center sm:gap-3 gap-0 ml-auto ">
  <button type='button'  className="mr-2 gap-3 flex hover:border font-serif text-white  bg-[#445279] hover:border-blue-400 hover:text-black rounded-xl py-2 px-5 hover:scale-110 hover:bg-[#002c7424] font-semibold duration-300" onClick={handleDownload}>
    ExporttoEXCEL
  </button> 
  </div>
            </div> 
            ...
            <Table>
              <TableHead >
                <TableRow className="text-sm tracking-wide text-center text-white bg-bg uppercase border-b border-blac">
                  ...
                  <TableCell className='px-4 text-center border-x border-black py-2 text-white'>Batch</TableCell>
                  <TableCell className='px-4  text-center border-x border-black py-2 text-white'>Comments</TableCell>
                  <TableCell className='px-4 text-center border-x border-black py-2 text-white'>Enrolled project</TableCell>
                  <TableCell className='px-4 text-center border-x border-black py-2 text-white'>CGPA</TableCell>
                  <TableCell className='px-4 text-center border-x border-black py-2 text-white'>Reason</TableCell>
              ...
              <TableBody>
                {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((student, index) => (<TableRow key={student.id} className={index % 2 === 0 ? "bg-[#88B7DF]" : "bg-[#FFFFFF]"}>
                  ...
                  <TableCell className="px-4 py-2 text-center text-sm border">   <ul>
                    {student.student_subjects.map((subjects) => {
                      return (
                        <li key={subjects.id}>
                          Grade:{subjects.grade}
                          {
                            subjects.Subject.map((Subject) => { return (<li key={Subject.id} value={Subject.title}>Subject:{Subject.title}</li>) })}
                        </li>
                      )
                    })}
                  ...
                  <TableCell className="px-4 py-2 text-center text-sm border">{student.Batch.name}</TableCell>
                  <TableCell className="px-4 py-2 text-center text-sm border">{student.comments}</TableCell>
                  <TableCell className="px-4 py-2 text-center text-sm border">{student.cgpa}</TableCell> 
                  <TableCell className="px-4 py-2 text-center text-sm border">{student.is_enrolled_projectt ? 'true' : 'false'} </TableCell>
                  <TableCell className="px-4 py-2 text-center text-sm border"> {student.reason}</TableCell>
